...
"Mozilla/5.0 (X11; Linux x86_64; rv:{0}.0) Gecko/{0}{1:02d} Firefox/{0}.0",
        ...
        print()
    if select == "21":
        pystyle.Write.Print("\n[+] Поддержать проект: USDT TRON – TRC20 TM2DpT4rcZabUNs1BF7Fty8RMvDeTowCPK  \n\n[+] ТГК -> https://t.me/MUSASHI_NO_WAT \n[+] ИНФОРМАЦИЯ: просто написал для новичков  ЗА ВАШИ ДЕЙСТВИЯ ОТВЕТСТВЕННОСТЬ НЕСЁТЕ ИСКЛЮЧИТЕЛЬНО ВЫ \n\n[+] ДАТА СОЗДАНИЯ СОФТА -> ЗАБЫЛ \n\n[!] СПАСИБО , ЧТО ЦЕНИТЕ СТАРАНИЯ\n", pystyle.Colors.green, interval = 0.005)
    if select == "22":
        pystyle.Write.Print("\n[+] СОЗДАТЕЛЬ @MUSASHI_TYPK \n", pystyle.Colors.green, interval = 0.005)
    if select == "23":
